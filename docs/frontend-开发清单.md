...
前端要做成一个可二次开发的管理台，而不是把某个业务扩展的页面硬写进通用框架里。

本次前端建设解决四件事：

1. 给现有 Admin API 提供可直接使用的管理台入口。
2. 把通用运行态、市场、订单、持仓、审计、人工操作做成稳定模块。
3. 把扩展相关展示收口到单独扩展位，不把策略规则揉进通用页面。
4. 目录、共享组件和 API 访问层已经按模块拆开；新增二开业务或新增页面时只需要改对应模块。
...
- 解释某个业务扩展如何看待 market 状态。
- 展示某个业务扩展专有的摘要、标签、说明和扩展面板。
- 在通用 market 数据上做扩展语义映射。
...
- `features/` 只能依赖 `shared/`、`core/` 和扩展展示接口，不能直接依赖具体扩展实现。
...
扩展展示位采用显式注册，而不是在通用页面里写 `if extension_module == "strategies.current"`。
...
- 已建立扩展展示注册表与 `strategies.current` 对应前端扩展模块。
- 已补充运行说明、构建验证和开发命令。
...
- 业务页面：`src/features/`。
- 扩展展示槽：`src/extensions/registry.ts` 与 `src/extensions/current/`。
