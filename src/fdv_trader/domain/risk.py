...
from fdv_trader.domain.allocation import AllocationPlan, current_exposure_usdc
from fdv_trader.domain.market import Market, TradingStatus
from fdv_trader.domain.order import Order, OrderIntent, OrderSide, OrderStatus
        ...
        *,
        ...
        if classification_passed is False:
            return self._fail(
                ...
                reason=classification_reason or "not_crypto_fdv_500m",
                field="classification",
                suggested_action="reject",
                retryable=False,
            )
        if classification_passed is None and market is not None:
...
def _market_has_target_classification(market: Market) -> bool:
    ...
    matched_tokens = {keyword.strip().lower() for keyword in market.matched_keywords}

    has_crypto = "crypto" in category_tokens or "cryptocurrency" in category_tokens
    has_fdv = "fdv" in matched_tokens or "fully diluted valuation" in matched_tokens
    has_500m = bool({"500m", "$500m", "500 million", "500,000,000"} & matched_tokens)
    return has_crypto and has_fdv and has_500m
