...
class PolymarketTradingClient:
    ...
    def create_signed_order(self, request: OrderExecutionRequest) -> Any:
        exported = _require_py_clob_client()
        client = self._ensure_client(require_l2=False)
        official_order_type = getattr(
            exported["order_type"],
            (request.order_type.value if request.order_type is not None else "GTC").upper(),
        )
    ...
    def _normalize_response(self, response: Any) -> Mapping[str, Any]:
        ...
        if "status" not in normalized:
            if normalized.get("error") or normalized.get("errorMsg") or normalized.get("success") is False:
                normalized["status"] = "rejected"
            elif normalized.get("canceled") is True or normalized.get("cancelled") is True:
...
class PolymarketOrderExecutionClient:
    ...
    def submit_order(self, request: OrderExecutionRequest) -> OrderExecutionResponse:
        ...
        return OrderExecutionResponse(
            ...
            reason=_text(response.get("reason")) or "submitted",
            retryable=False,
        )
