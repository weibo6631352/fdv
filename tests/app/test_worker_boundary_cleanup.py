...
def test_ws_workers_delegate_external_payload_parsing_to_adapters() -> None:
    ...
    user_source = Path("src/polymarket_trader/workers/user_ws_worker.py").read_text(encoding="utf-8")

    assert "market_ws_adapter" in market_source
    assert "user_ws_adapter" in user_source
    assert "def _parse_levels" not in market_source
    assert "def _iter_order_snapshots" not in user_source
