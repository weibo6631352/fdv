...
- 当前仍处于开发阶段，数据库 schema 调整优先直接改模型和初始化逻辑，不预留历史 schema 包袱。
- 开发阶段应尽量避免把历史遗留问题、兼容包袱或过早抽象带入当前实现；除非已有明确线上约束，否则优先保持结构简单、规则直接、可重建。
- 开发者需要补充标准、清晰、可维护的文档注释和代码注释；新增模块、公共函数、核心流程、并发控制和外部接口适配时，应写清职责、关键约束、输入输出、副作用、异常处理和必要的业务背景，并与实现保持同步。
...
- 是否补充或确认了对应审计字段，并同步更新了模块文档、文档注释和代码注释，按注释规范解释关键业务规则与异常处理原因。
