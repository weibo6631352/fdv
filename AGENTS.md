...
本文件整理自仓库内的 `README.md`、`docs/需求文档.md`、`docs/设计文档.md`、`docs/全量市场发现改造计划.md` 以及各层 `README`，用于约束 Codex 在本仓库中的改动方式。
...
- 兼容层只在明确存在外部契约约束时保留，并且必须记录原因、边界和移除条件。
- 发现调用侧仍使用旧名字时，优先修改调用侧，不要在框架层长期保留同义字段、别名、包装函数或重复枚举。
...
- 契约字段命名必须单义，避免一套名字对外、一套名字对内的长期兼容层。
